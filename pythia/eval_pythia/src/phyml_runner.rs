...
pub fn run(aln_path: &PathBuf) -> f64 {
    let aln_path = convert_to_phylip(aln_path);
    match aln_path {
        Some(aln_path) => {
            ...
            if phyml_output.status.success() {
                println!(
                    "Phyml ran successfully in {} seconds",
                    runtime.as_secs_f64()
                );
                runtime.as_secs_f64()
            } else {
                eprintln!(
                    ...
                    String::from_utf8(phyml_output.stderr).unwrap()
                );
                -1.0
            }
        }
...
fn convert_to_phylip(aln_path: &PathBuf) -> Option<PathBuf> {
    ...
    let mut phy_path = aln_path.clone();
    ...
    if converter_output.status.success() {
        ...
        eprintln!(
            ...
            String::from_utf8(converter_output.stderr).unwrap()
        );
        None
    }
}
